...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        assert T % self.tubelet_size == 0, f"Number of frames {T} is not divisible by tubelet size {self.tubelet_size}"
        # B, C, T, H, W -> B, embed_dim, T // tubelet_size, H // patch_h, W // patch_w
        x = self.proj(x)
        # B, embed_dim, T // tubelet_size, H // patch_h, W // patch_w ->
        # B, embed_dim, (T // tubelet_size) * (H // patch_h) * (W // patch_w) ->
        # B, (T // tubelet_size) * (H // patch_h) * (W // patch_w), embed_dim
